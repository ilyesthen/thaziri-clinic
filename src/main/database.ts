...
import { getDatabaseUrl, ensureDatabaseAccess } from './utils/database-path'
import { fixPrismaPath } from './utils/prisma-fix'
import { loadPrismaClient, setupWindowsPrismaEnvironment, getDatabasePath } from './utils/prisma-client-loader'
import { app } from 'electron'

// Load PrismaClient dynamically for cross-platform support
let PrismaClient: any;
try {
  const PrismaModule = loadPrismaClient();
  PrismaClient = PrismaModule.PrismaClient || PrismaModule;
} catch (error) {
  ...
  try {
    ...
    throw new Error('Cannot initialize PrismaClient');
  }
}
...
export function getPrismaClient(): any {
  if (!prisma) {
    ...
    setupWindowsPrismaEnvironment();
    
    // Fix Prisma paths for production (legacy support)
    fixPrismaPath();
    ...
    console.log(`🗄️  Using database: ${databaseUrl}`);
...
export async function initializeDatabase(): Promise<void> {
  // First, ensure we have access to the database file
  if (!ensureDatabaseAccess()) {
  ...
  try {
    ...
    // If it's a fresh database, Prisma will create the schema automatically
    console.log('📝 Will create fresh database on first query')
  }
}
