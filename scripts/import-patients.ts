...
async function importPatients() {
  ...
  try {
    ...
    // Import patients in batches using transactions
    console.log('\n💾 Importing patients into database...')
    ...
    for (let i = 0; i < totalBatches; i++) {
      ...
      const end = Math.min(start + BATCH_SIZE, transformedPatients.length)
      const batch = transformedPatients.slice(start, end)
      ...
      try {
        ...
        process.stdout.write(`\r   Progress: ${progress}% (${totalImported.toLocaleString()} / ${transformedPatients.length.toLocaleString()} records)`)
      } catch (error) {
    ...
    console.log(`\n✨ Successfully imported ${totalImported.toLocaleString()} patient record(s)`)
